...
shinyUI(fluidPage(
  fluidPage(
    titlePanel("Finding Drug/Side Effect Correlation"),
    sidebarLayout(
      sidebarPanel(
      ...
      mainPanel(
        ...
        tabsetPanel(
          tabPanel(h4("Overview"),
                   ...
                   the side effects associated with the given drug.", br(), br(),
                   "Our second plot tracks the inverse; when the user gives us side effects they are experiencing, 
                   we return the drugs which are associated with said side effects.",  br(), br(), 
                   "Finally, we depict a map of health clinics in Seattle for users to refer to in case they
